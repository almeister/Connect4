...
public class Board {
    ...
    public void makeMove(Move move) throws IllegalArgumentException {
        if (board[0][move.getColumn()] != null) {
            throw new IllegalArgumentException("Cannot place a disc in this column because it is full.");
        }

        // Fill lowest empty position with player's disc
        for (int row = board.length - 1; row >= 0; row--) {
            if (getPlayer(row , move.getColumn()) == null) {
                setTile(move, row);
                break;
            }
        }
    ...
    private void setTile(Move move, int row) {
        assert 0 <= row  &&  row < NUM_ROWS  &&  0 <= move.getColumn()  &&  move.getColumn() < NUM_COLS;

        board[row][move.getColumn()] = move.getPlayer();
    }
...
//        Board has a winner already
        if (hasConnectFour() != null) {
            return new Move[0];
        }

        Vector<Move> tempMoves = new Vector<>();

        for (int col = 0; col < NUM_COLS; col++) {
            if (isColumnFull(col) == false) {
                tempMoves.add(new Move(p, col));
            }
        }

        Move[] moves = new Move[tempMoves.size()];
