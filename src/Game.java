...
public class Game {
    ...
    public static void testFillBottomRowAndGetPossibleMoves() {
        Board b= new Board();
        Board.Player p = Board.Player.RED;

        for (int c = 0; c < b.NUM_COLS; c++) {
            b.makeMove(new Move(p, c));

            if (p == Board.Player.RED) {
                p = Board.Player.YELLOW;
            } else {
                p = Board.Player.RED;
            }
        }
    ...
    public static void testFillAllColumnsAndGetPossibleMoves() {
        Board b= new Board();
        Board.Player p = Board.Player.RED;
        int col = 0;
    ...
    public static void testInitaliseStateChildrenInEmptyBoard() {
        System.out.println("testInitaliseStateChildrenInEmptyBoard");
        Board b= new Board();
        State state = new State(Board.Player.RED, b, null);

        state.initializeChildren();
        State[] stateChildren = state.getChildren();

        for (State s : stateChildren) {
            if (s != null) {
    ...
    public static void testInitaliseStateChildrenInFullBoard() {
        System.out.println("testInitaliseStateChildrenInFullBoard");
        Board b= new Board();
        Board.Player p = Board.Player.RED;
        int col = 0;
        ...
        State state = new State(p, b, null);
    ...
    public static void testCreateGameTree() {
        System.out.println("testCreateGameTree");
        Board b= new Board();
        Board.Player p = Board.Player.RED;
        final int DEPTH = 4;
        AI ai = new AI(p, DEPTH);
        int col = 0;
...
//        System.out.println(state.toString());
    ...
    public static void testMiniMax() {
        System.out.println("testMiniMax");
        Board b= new Board();
        Board.Player p = Board.Player.RED;
        final int DEPTH = 4;
        AI ai = new AI(p, DEPTH);
    ...
    public static void fillColumn(Board board, Board.Player player, int col) {
        ...
        for (int row = 0; row < Board.NUM_ROWS; row++) {
            ...
            board.makeMove(new Move(currentPlayer, col));
...
//        testFillBottomRowAndGetPossibleMoves();
//        testFillAllColumnsAndGetPossibleMoves();
//        testInitaliseStateChildrenInEmptyBoard();
//        testInitaliseStateChildrenInFullBoard();
//        testCreateGameTree();
        testMiniMax();
    }
